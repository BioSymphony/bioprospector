...
tool outputs into the ledger shapes validated by
``bioprospector_genecluster_atlas_contracts.py`` without invoking plantiSMASH,
cblaster, CLEAN, Foldseek, OrthoFinder, GENESPACE, or any other external
program.
"""
...
def build_protein_function_jury(vote_rows: Sequence[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    for protein_id in sorted(grouped):
        ...
        top_label = ranked[0]
        supporting = sorted(set(label_tools[top_label]))
        contradicting = sorted({tool for label in ranked[1:] for tool in label_tools[label] if tool})
        support_confidences = label_scores[top_label]
