...
BioProspector is useful because the same campaign contract can grow from a
one-hour local demo into a tracker-backed, cloud-ready bioprospecting program.
Start local, generate concrete artifacts, then promote the same ledgers into
agent work graphs, Linear-style issue lanes, provider readiness packets, and
claim-audited closeouts.
...
| --- | --- | --- | --- |
| Local first hour | You want to see the system produce useful artifacts immediately. | Demo dossier, campaign graph, candidate package, and Pareto ranking under `.runtime/local-demo/`. | None. |
...
| Agent kickoff brief | You have Codex, Claude Code, Symphony/Linear, or `/goal` and want a strong starting packet. | Ready-to-paste prompt, local commands, lane counts, and safety boundaries. | None. |
...
| Operator-owned live run | You already approved budget, data policy, credentials, storage, and cleanup outside the repo. | Compact execution artifacts, evidence joins, and maturity/claim audit. | User-managed provider environment. |
| Public release check | You want the repo itself to be publishable later. | Release gate, docs checks, audits, gitleaks output. | None required. |

## 1. Local First Hour

This is the default way to evaluate BioProspector. It proves the repo can build
the core artifact chain before any external service enters the picture.
...
Use this when you want a campaign packet for a new molecule/host. This creates
the scaffolding an agent can reason over before it expands routes, mines enzyme
...
This stays local. The generated Markdown is useful for:
...
| Pareto ranking | Return several useful route winners instead of one brittle answer. | `ranking-*.md` | candidate package | first wave for examples | `pareto-frontier-ledger.tsv` |
...
Generate a campaign-specific agent kickoff brief when the user already has a
capable orchestrator and mainly needs the right prompt, commands, and
boundaries:
...
The brief complements agent orchestration. It does not call trackers, launch
cloud resources, or replace a Symphony/Linear workflow.

## 4. Linear Or Tracker Mirror

Use this when a team wants the BioProspector work graph in Linear, GitHub
Issues, Jira, or another tracker. The public repo does not call tracker APIs.
...
- only the first validated wave is active
- every issue keeps the `Claim Boundary`, `Search Budget`, and `Kill Criteria`

Linear is useful here because it can hold dependencies, ownership, blockers,
review gates, and closeout comments. BioProspector still remains useful without
Linear because the source of truth is the local campaign contract and generated
ledger set.
...
| Status | `Backlog` by default; `Active` only for the first validated wave. |
| Dependencies/blockers | Draft `Depends On`, `Review Gate`, and provider-preflight rows. |
...
| Evidence links | Compact `.runtime/` sidecar paths, accessions, checksums, and dossier path. |
...
- how a failed or partial provider run should close without losing useful
...
| `local_full` | User-owned hardware with external workdirs. | stage contract plus external path policy | compact ledgers and checksums | Data would be copied into git. |
...
| `managed_workflow` | Future repeatable production-style runs. | workflow-framework ledger | normalized workflow artifacts and proof rows | Wrappers cannot emit BioProspector ledgers. |
...
- query/input rows point to approved public or secure external data
- credentials stay in provider-side secret stores or user shells, never in repo
...
Use this when preparing the repo for a future public switch while still staying
local.
